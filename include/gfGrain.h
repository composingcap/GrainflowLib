	...
	public:
		...
		inline void Process(gfIoConfig &ioConfig) {
			...
			for (int i = 0; i < ioConfig.blockSize / BLOCKSIZE; i++)
			{
				...
				OuputBlock(sampleIdTemp, ampTemp, densityTemp, bufferInfo.oneOverBufferFrames, stream, inputAmp, grainPlayhead, grainAmp, grainEnvelope, grainOutput, grainChannels, grainStreams, BLOCKSIZE);
			}
		}
        ...
        inline GfValueTable* GrainReset(double* __restrict grainClock, const double* traversal, double* __restrict grainState, const int size)
		{
			...
			if (!grainReset) return valueTable;
				...
				if(!bufferReader.SampleParamBuffer(GetBuffer(GFBuffers::windowBuffer), ParamGetHandle(GfParamName::window),g)) SampleParam(GfParamName::window);
				SampleParam(&space);
				SampleParam(&glisson);
		...
		void SetBuffer(GFBuffers bufferType, T *buffer)
		{
			switch (bufferType)
			{
		...
		T *GetBuffer(GFBuffers bufferType)
		{
			switch (bufferType)
			{
			case (GFBuffers::buffer):
        ...
        inline void Increment(const double* __restrict fm, const double* __restrict grainClock, double* __restrict samplePositions, double* __restrict sampleDeltaTemp, const int size)
		{
			...
			double end = std::min((double)bufferInfo.bufferFrames * stopPoint.value, (double)bufferInfo.bufferFrames - 1);
