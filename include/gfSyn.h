...
namespace Grainflow {
	...
	public:
		...
		template<long INTERNALBLOCK>
		static inline bool SineTable(const double* freqs, double** __restrict outputs, const long blockSize, const long channels, const double oneOverSamplerate, double* __restrict positions, double* __restrict extra, double* __restrict history) {
			if (blockSize < INTERNALBLOCK) return false;
			long blocks = blockSize / INTERNALBLOCK;
			//This assumes the same frequency over the entire block
			for (long ch = 0; ch < channels; ch++) {
				for (long i = 0; i < blocks; i++) {
					...
					GfSyn::ReadQuarterTable<double, INTERNALBLOCK>(gf_envelopes::quarter_sine_wave, result, positions, extra, 4096);
				}
			}
			return true;
		}
		...
		template<long INTERNALBLOCK>
		static inline bool ChevySine(const double* freqs, double** __restrict outputs, const long blockSize, const long channels, const double oneOverSamplerate, double* __restrict positions, double* __restrict history) {
			if (blockSize < INTERNALBLOCK) return false;
			long blocks = blockSize / INTERNALBLOCK;
			//This assumes the same frequency over the entire block
			for (long ch = 0; ch < channels; ch++) {
		...
		template<typename T, long INTERNALBLOCK>
		static inline void ReadTableLerp(const T* __restrict table, T* __restrict result, const float* __restrict positions) {
		...
		template<typename T, long INTERNALBLOCK>
		static inline void ReadQuarterTable(const T* __restrict table, T* __restrict result, T* __restrict positions, T* __restrict extra, const int size) {
			for (long i = 0; i < INTERNALBLOCK; ++i) {
